...
void ReplyComm::setEncryptEngines(const RSA_Encryptor& rsaEn, const RSA_Decryptor& rsaDe, const AES_Encryptor& aesEs) {
  rsaEn_ = rsaEn;
  rsaDe_ = rsaDe;
  aesEn_ = aesEn;
...
void ReplyComm::recvMultipleResponse() {
  ...
  string cipher = recvBuffer_;
  string recovered = aesDecryptor.decrypt(cipher);      
  JsonParser jsonDate(recovered.c_str());

  //jsonData.deserializer();

  string className = jsonData.getString("ClassName"); //try 문 작성 안하면 터짐

  if(!className.compare("Request")) {
    //userid check
    string userID = jsonData.getString("UserID");    
    if(g_userID == userID){
      string pubkey = rsa_decryptor.getPublicKey();
      string deviceID = g_deviceID;
      //json 에 담아야 함
      //암호화
      UDPComm_.unicast(c_addr_,rsa_decryptor.getPublicKey());
    }
  }
  else if(!className.compare("Response")) {

    //device id 저장  
    string deviceID = jsonData.getString("DeviceID");
    string publicKey = jsonData.getString("PubKey");
    g_IPtoDeviceTable[deviceID] = c_addr_;
    pubKeyStore->addDevice(deviceID, publicKey);
  ...
  else(!className.compare("UserActivity")){
    UserActivity->deserializer(JsonData);
    //UserActivity 저장
    //Device 별 UserActivity hashmap 저장
  }
}
